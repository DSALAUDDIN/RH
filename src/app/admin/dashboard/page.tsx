...
export default function AdminDashboard() {
  ...
  const fetchReviews = useCallback(async () => {
    ...
    if (res.ok) {
      const data: { reviews: VideoReview[] } = await res.json();
  ...
  useEffect(() => {
    ...
    fetch('/api/reviews')
      ...
      .then((data: { reviews: VideoReview[] } | null) => {
        if (!cancelled && data) setReviews(data.reviews);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        'Are you sure you want to delete this video? It will be removed from the database and Cloudinary.',
      )
    )
      return;
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <header style={{ marginBottom: '40px' }}>
            ...
            <p style={{ color: '#64748b', marginTop: '8px' }}>
              Manage patient testimonials and upload new success stories.
            </p>
          </header>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '32px' }}>
            ...
            >
              <h2 style={{ fontSize: '1.25rem', marginBottom: '24px', color: '#0f172a' }}>
                Upload New Review
              </h2>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                {reviews.length === 0 && (
                  <p style={{ color: '#94a3b8', textAlign: 'center', padding: '40px' }}>
                    No reviews found. Upload one to get started.
                  </p>
                )}
              </div>
            </div>
