...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const author = DOCTORS[post.author ?? DEFAULT_AUTHOR];
  return pageMeta({
    title: post.title,
    ...
    authors: author ? [author.fullName] : undefined,
  ...
  switch (section.type) {
    case 'heading':
      return (
        ...
        >
          <span
            style={{ width: '4px', height: '1.2em', background: accentColor, borderRadius: '4px' }}
          />
          {section.content}
        </h2>
      );
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const structuredData = [
    ...
    breadcrumbs({ name: 'Articles', path: '/blog' }, { name: post.title, path }),
  ];
